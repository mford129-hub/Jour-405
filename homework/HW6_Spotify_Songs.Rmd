...
```{r}
hist(spotify$danceability)
abline(v=mean(spotify$danceability))
```
...
When I look at the danceability scores are slightly left-skewed rather than perfectly normal. Most songs have danceability scores between about 0.5 and 0.9. The mean score is about 0.67, which shows that the songs in the dataset are generally fairly danceable.
